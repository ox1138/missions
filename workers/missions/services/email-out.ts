...
async function deliverViaBinding(env: Env, d: DeliverInput): Promise<boolean> {
	...
	try {
		...
		// The CF Email Service binding throws errors with a `.code` property
		// (see workers/email-sender.ts). The default Error.message strips
		// that, so we rebuild a useful error string from the whole object.
		...
		// Also dump every enumerable property in case the real info is elsewhere.
		try {
